 ...
 *
 * ## Why this is a second copy rather than a shared emit script
 *
 * It is a near-duplicate of `packages/contracts-service-a/scripts/emit.ts`,
 * and deliberately so, for the same reason `../src/schemas/shared.ts` restates
 * primitives rather than importing them. The two contract packages are
 * versioned independently (spec section 2.2), so a shared build script would
 * make every edit to `service-a`'s emit metadata a candidate version bump of
 * this package — and every `service-b` consumer would then review a change to
 * an API it does not call. What the two share is `@marcos-corp/contract-tooling`,
 * which is the gate machinery rather than either published surface.
 *
 ...
 *   not what the API promises, and this document is read by `web-b` and by the
 *   Prism mock — two callers with two different answers. ts-rest clients take a
 *   `baseUrl` and Prism takes a host and port, so nothing here needs it, and a
 *   hardcoded `localhost` in a published contract is a fact about one laptop.
 *
 * The description says out loud that this provider is also a consumer, because
 * that is the whole reason `service-b` exists in spec section 1 and a reader
 * of the published document would otherwise have no way to tell.
 */
function emitMeta(): EmitMeta {
  return {
    info: {
      ...
      description:
        'Invoices for the company dashboard banner. service-b publishes this '
        + 'contract while pinning @marcos-corp/contracts-service-a at an exact '
        + 'version to resolve company context, so the same team authors a '
        + 'contract change on one side and approves one on the other. '
        + 'Consumers pin this package exactly. See docs/governance.md.',
